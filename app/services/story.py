...
class StoryService(Repository):
    ...
    def getStory(cls, auth):
        ...
        return createSuccessResponse(story[0].toJSON(topics=res, reposted=reposted, favorited=favorited, user=story[1].toJSON(), reposts=story[2], favorites=story[3]))
    ...
    def getStories(cls, userId, auth):
        ...
        for story in stories:
            ...
            res.append(story[0].toJSON(own=own, topics=resTopics, reposted=reposted, favorited=favorited, user=story[1].toJSON(),
                reposts=story[2], favorites=story[3]))
    ...
    def getFavoriteStories(cls, userId, auth):
        ...
        for story in stories:
            ...
            for topic in topics:
                resTopics.append(topic.toJSON())
            res.append(story[0].toJSON(own=own, topics=resTopics, reposted=reposted, favorited=True,
                                       user=story[1].toJSON(),
                                       reposts=story[2], favorites=story[3]))
